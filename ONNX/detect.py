...
ort_sess = ort.InferenceSession('./yolov5m.onnx')
...
def preprocess_image(raw_bgr_image):
    input_size = (640, 640) # h,w = 640,640
...
def inference(img, classes):
    _img = preprocess_image(copy.deepcopy(img))
    _img = _img.transpose(0, 3, 1, 2) # NHWC to NWHC
    output = ort_sess.run(None, {
        'images': _img
    })
    detections = output[0]
    detections = non_max_suppression(torch.Tensor(detections), 0.25, 0.45, max_det=1000)
    detections = detections[0]
    num_detections, nmsed_boxes, nmsed_scores, nmsed_classes = detections.shape[0], detections[:, 0:4], detections[:, 4:5], detections[:, 5:]
    
    colors = Colors()
    result_image = np.squeeze(img)
